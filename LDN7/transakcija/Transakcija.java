...
public class Transakcija {
    ...
    public Transakcija(DataInputStream xmlInput) {
        ...
        try {
            ...
            System.out.println("Napaka pri branju/parsanju dokumenta!");
        }
    }
    ...
    private void init(TipTransakcije tip, String stranka, Artikel[] artikli,
                      String odgovor) {
        ...
        if (tip == TipTransakcije.Inventura && odgovor != null && !odgovor.equals("")) {
    ...
    private void generateXMLDocument() {
        ...
        Element elZadnji = _artikli[_artikli.length - 1].getXMLElement(_doc);
        elTransakcija.appendChild(elZadnji);

        for (int i = _artikli.length - 2; i >= 0; i--) {
            Element el = _artikli[i].getXMLElement(_doc);
    ...
    public Transakcija generateOdgovor() {
        ...
        return setOdgovor(odgovor.toString());
    }
    ...
    void writeToXMLFile(File f) throws IOException {
