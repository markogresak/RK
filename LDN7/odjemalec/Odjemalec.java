...
public class Odjemalec {
	...
	private String preberiPodatke() throws IOException {
		...
		do {
			if (tipString != null)
				System.out
						.println("Tip transakcije ne obstaja! Poizkusite znova.");
		...
		do {
			if (stranka != null)
				System.out
						.println("Vneseno ime ne sme biti prazen niz! Poizkusite znova.");
		...
		if (tip != TipTransakcije.Inventura) {
			...
			do {
				if (artikel != null)
					System.out
							.println("Vnesen artikel ne sme biti prazen niz! Poizkusite znova.");
			...
			do {
				if (!kprvic)
					System.out
							.println("Stevilo artiklov mora biti pozitivno! Poizkusite znova.");
			...
			do {
				if (!iprvic)
					System.out
							.println("ID skladisca mora biti pozitiven! Poizkusite znova.");
				System.out.print("Vnesite ID skladisca: ");
				idSkladisca = Integer.parseInt(reader.readLine().trim());
				iprvic = false;
			} while (idSkladisca <= 0);
			new Transakcija(tip, stranka, new Artikel[] { new Artikel(artikel,
					kolicina, idSkladisca) }).getXMLDocumentString();
		}

		return new Transakcija(tip, stranka, new Artikel[] { Artikel.empty() })
